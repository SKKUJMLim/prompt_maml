...
from prompters import PromptGenerator, PromptAdapter
...
class MAMLFewShotClassifier(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        channel = 3
        self.arbiter = PromptGenerator(args=self.args, device=device,
                                       nz=nz, ngf=64, img_size=img_size, nc=channel).to(device=self.device)


        arbiter_weights_copy = self.get_inner_loop_parameter_dict(self.arbiter.named_parameters())
        arbiter_weights_copy = {key: value for key, value in arbiter_weights_copy.items() if 'prompt_dict' in key}

        self.arbiter_adapter = PromptAdapter(input_dim=self.args.num_text_embedding_params,
                                             num_prompt_generator_layers=len(arbiter_weights_copy),
                                             args=args, device=device).to(device=self.device)
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase,
        ...
        for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in \
            ...
            names_weights_copy = self.get_inner_loop_parameter_dict(self.classifier.named_parameters())
            prompted_weights_copy = self.get_inner_loop_parameter_dict(self.arbiter.named_parameters())
            ...
            names_weights_copy = {key: value for key, value in names_weights_copy.items() if 'layer_dict' in key}
            prompted_weights_copy = {key: value for key, value in prompted_weights_copy.items() if 'prompt_dict' in key}

            # print("names_weights_copy === ", names_weights_copy.keys())
            # print("prompted_weight_dict 1 === ", prompted_weights_copy.keys())
            ...
            z = torch.randn(size=[1, self.args.num_text_embedding_params], requires_grad=True).to(self.device)

            for num_step in range(num_steps):

                ideal_prompt = self.arbiter(task_embedding=z, prompted_params=prompted_weights_copy)
                ...
                gradients = torch.autograd.grad(support_loss, (*names_weights_copy.values(), *prompted_weights_copy.values(), z),
                ...
                prompted_weights_copy = self.arbiter_adapter(z, num_step, prompted_weights_copy)
