...
class VGGReLUNormNetwork(nn.Module):
    def __init__(self, im_shape, num_output_classes, args, device, meta_classifier=True):
        ...
        # (NEW) feature-space prompter
        self.feature_prompter_enabled = getattr(self.args, "feature_prompter", False)
        if self.feature_prompter_enabled:
            # args.feature_prompt_layers = None 이면 전 stage에 적용한다고 가정
            self.feature_prompt_layers = getattr(self.args, "feature_prompt_layers", None)
            self.feature_prompt = prompters.FeatureSpacePrompter(args).to(device)
    ...
    def build_network(self):
        ...
        for i in range(self.num_stages):
            self.layer_dict[f'conv{i}'] = MetaConvNormLayerReLU(
            ...
            out = self.layer_dict[f'conv{i}'](out, training=True, num_step=0)

            # maxpool이 있으면 feature map 크기 변하므로 여기서 반영
            if self.args.max_pooling:
                out = F.max_pool2d(input=out, kernel_size=(2, 2), stride=2, padding=0)

            # (NEW) prompting을 적용할 layer의 shape 기록
            if self.feature_prompter_enabled:
                layers = self.feature_prompt_layers  # None이면 all
                if (layers is None) or (i in layers):
                    feature_shapes[i] = (out.shape[1], out.shape[2], out.shape[3])

        # (NEW) 이제 shape를 알았으니 prompt 파라미터 생성
        if self.feature_prompter_enabled:
            self.feature_prompt._build_if_needed(feature_shapes)
    ...
    def forward(self, x, num_step, params=None, prompted_params=None,
                training=False, backup_running_statistics=False,
                task_embedding=None, prepend_prompt=True):
        ...
        for i in range(self.num_stages):
            out = self.layer_dict[f'conv{i}'](
                out,
                params=param_dict[f'conv{i}'],
                training=training,
                backup_running_statistics=backup_running_statistics,
                num_step=num_step
            )
            ...
            if self.feature_prompter_enabled:
                ...
                if (layers is None) or (i in layers):
                    out = self.feature_prompt(out, layer_idx=i, prompted_params=prompted_params)
